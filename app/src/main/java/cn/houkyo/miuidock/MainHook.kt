...
class MainHook: IXposedHookLoadPackage,IXposedHookInitPackageResources {
    companion object {
        ...
        const val DEVICELEVEL_UTILS_CLASSNAME = "$MIUI_HOME_LAUNCHER_PACKAGENAME.launcher.common.DeviceLevelUtils"
        const val CPULEVEL_UTILS_CLASSNAME = "$MIUI_HOME_LAUNCHER_PACKAGENAME.launcher.common.CpuLevelUtils"
        // 单位dip
    ...
    override fun handleInitPackageResources(resparam: XC_InitPackageResources.InitPackageResourcesParam) {
        ...
        resparam.res.hookLayout(MIUI_HOME_LAUNCHER_PACKAGENAME, "layout", "layout_search_bar", object : XC_LayoutInflated() {
            override fun handleLayoutInflated(liparam: LayoutInflatedParam) {
                // 替换资源圆角
                val targetView = liparam.view
                drawableNameList.forEach {
                    drawableName -> resetDockRadius(resparam.res, targetView.context, drawableName)
                }
            }
        })
    }
    ...
    private fun launcherHook(lpparam: XC_LoadPackage.LoadPackageParam){
        val _LAUNCHER_CLASS = XposedHelpers.findClassIfExists(LAUNCHER_CLASSNAME, lpparam.classLoader) ?: return
        try {
            XposedHelpers.findAndHookMethod(_LAUNCHER_CLASS, "onCreate", Bundle::class.java, object : XC_MethodHook() {
                override fun afterHookedMethod(param: MethodHookParam) {
                    super.afterHookedMethod(param)
                    val SearchBarObject = XposedHelpers.callMethod(param.thisObject, "getSearchBar") as FrameLayout
                    ...
                    // 修改宽度
                    SearchBarObject.layoutParams.height = Utils().dip2px(SearchBarDesktop.context, 90)
                    ...
                    SearchBarDrawer.layoutParams.height = Utils().dip2px(SearchBarDesktop.context, 45)
    ...
    private fun deviceConfigHook(lpparam: XC_LoadPackage.LoadPackageParam){
        val _DEVICE_CONFIG_CLASS = XposedHelpers.findClassIfExists(DEVICE_CONFIG_CLASSNAME, lpparam.classLoader) ?: return
        try {
            // 图标区域顶部边距
            XposedHelpers.findAndHookMethod(_DEVICE_CONFIG_CLASS, "calcHotSeatsMarginTop", Context::class.java, Boolean::class.java, object : XC_MethodHook() {
            ...
            // 图标区域底部边距
            XposedHelpers.findAndHookMethod(_DEVICE_CONFIG_CLASS, "calcHotSeatsMarginBottom", Context::class.java, Boolean::class.java, Boolean::class.java, object : XC_MethodHook() {
                override fun beforeHookedMethod(param: MethodHookParam) {
                    super.beforeHookedMethod(param)
            ...
            // 搜索框宽度
            XposedHelpers.findAndHookMethod(_DEVICE_CONFIG_CLASS, "calcSearchBarWidth", Context::class.java, object : XC_MethodHook() {
                override fun beforeHookedMethod(param: MethodHookParam) {
                    super.beforeHookedMethod(param)
                    val _context = param.args[0] as Context
                    val deviceWidth = Utils().px2dip(_context, _context.resources.displayMetrics.widthPixels)
                    param.result = Utils().dip2px(_context, deviceWidth - 30)
                }
            })
            // 搜索框底部边距
            XposedHelpers.findAndHookMethod(_DEVICE_CONFIG_CLASS, "calcSearchBarMarginBottom", Context::class.java,Boolean::class.java, object : XC_MethodHook() {
                override fun beforeHookedMethod(param: MethodHookParam) {
                    super.beforeHookedMethod(param)
                    val _context = param.args[0] as Context
                    param.result = Utils().dip2px(_context, 20)
                }
            })
        }catch (e: Exception){
    ...
    private fun deviceLevelHook(lpparam: XC_LoadPackage.LoadPackageParam){
        val _BLUR_UTILS_CLASS = XposedHelpers.findClassIfExists(BLUR_UTILS_CLASSNAME, lpparam.classLoader) ?: return
        val _DEVICELEVEL_UTILS_CLASS =  XposedHelpers.findClassIfExists(DEVICELEVEL_UTILS_CLASSNAME, lpparam.classLoader) ?: return
        val _CPULEVEL_UTILS_CLASS =  XposedHelpers.findClassIfExists(CPULEVEL_UTILS_CLASSNAME, lpparam.classLoader) ?: return
        try {
            // 修改高斯模糊和动画等级
            XposedHelpers.findAndHookMethod(_BLUR_UTILS_CLASS, "getBlurType", XC_MethodReplacement.returnConstant(2))
            // 打开文件夹时是否模糊
            // XposedHelpers.findAndHookMethod(_BLUR_UTILS_CLASS, "isUserBlurWhenOpenFolder", XC_MethodReplacement.returnConstant(true))
            // 修改设备等级 2为最高级
            XposedHelpers.findAndHookMethod(_DEVICELEVEL_UTILS_CLASS, "getDeviceLevel", XC_MethodReplacement.returnConstant(2))
            ...
            // 是否使用的简单动画
            XposedHelpers.findAndHookMethod(_DEVICELEVEL_UTILS_CLASS, "isUseSimpleAnim", XC_MethodReplacement.returnConstant(false))
            // 下载应用动画
            XposedHelpers.findAndHookMethod(_CPULEVEL_UTILS_CLASS, "needMamlDownload", XC_MethodReplacement.returnConstant(true))
            // 骁龙CPU等级
            XposedHelpers.findAndHookMethod(_CPULEVEL_UTILS_CLASS, "getQualcommCpuLevel", String::class.java, XC_MethodReplacement.returnConstant(2))
    ...
    private fun resetDockRadius(res: XResources, context: Context, drawableName: String){
        res.setReplacement(MIUI_HOME_LAUNCHER_PACKAGENAME, "drawable", drawableName, object : XResources.DrawableLoader() {
            override fun newDrawable(xres: XResources, id: Int): Drawable {
                val background = ContextCompat.getDrawable(context, xres.getIdentifier(drawableName, "drawable", MIUI_HOME_LAUNCHER_PACKAGENAME)) as RippleDrawable
                val backgroundShape = background.getDrawable(0) as GradientDrawable
                backgroundShape.cornerRadius = Utils().dip2px(context, DOCK_RADIUS).toFloat()
